...
# to change directory getwd('~/the new directory')

#devtools::install_github("hadley/tidyr")
# loading package

# to find the missing data >>> is.na(p1)

# to omit missing data >>>  na.omit(p1)

#Imoprt the participants csv file named "001demo_visual_demo1_2019_May_31_1409" change the name to p1

# install.packages("tidyr")

 # download library(data.table) to create date set
#library(data.table)
...
analysis_A <- p1_auditory[blank_A]
 ...
 # first install.packages("plyr")

ddply(analysis_A, .(stim_ending_t, visbility, soundvolume), colwise(mean, .(key_resp_2.rt)))
...
analysis_A %>%
  ...
  mutate(key = case_when(visbility == 1 & soundvolume == 0 ~ "only_image",
                         visbility == 0 & soundvolume == 1 ~ "only_sound",
                         visbility == 0 & soundvolume == 0 ~ "blank")) %>%
